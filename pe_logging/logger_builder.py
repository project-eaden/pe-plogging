...
def create_file_handler(level: _level_types, file_path: Path) -> logging.Handler:
    try:
...
def create_stream_handler(level: _level_types, file_path: Optional[Path]) -> logging.Handler:
...
class PELogger:
    ...
    Args:
        ...
        - `level`[optional]: A **list** of the logging level(s) to use for the returned
        logger instance. If you'd like to specify a logging level per handler, then 
        ensure that the order is the same as that of the list `handler_type`. If 
        only only level is specified, then it will be applied to all the handlers
        of the resulting logger. The default level applied to all handlers is `logging.INFO`
        - `handler_type`[optional]: The **list** of desired handlers for the resulting 
        logger instance. Currently supported types are `stream` and `file`. The default 
        is a stream handler.
        - `file_path`[optional]: The path containing the logging file to write logs from 
        the filehandler to. Required only when a filehandler is requested. Recommend a 
        file ending in the extension .log
    """
    ...
    def __init__(
        ...
        self.handlers = [
            self._handler_types_map[ht](ll, file_path) for ht, ll in zip(handler_types, _levels)
        ]
    ...
    def __post_init__(self) -> None:
        if "file" in self.handler:
            assert self.file_path, \
                "No filepath specified for {name} FileHandler!"
        ...
        for h in self.handler:
            assert h in self.accepted_types, \
                f"Invalid handler type, {h}, passed! Valid options are 'stream' and 'file'."